                ...
                if ($newpass == $confirmpass) {
                    ...
                    if ($updatePass) {
                        $_SESSION['notification'] = [
                            'type' => 'success', // SweetAlert icon
                            'message' => 'Password Changed!' // SweetAlert title
                        ];
                    } else {
                        $_SESSION['notification'] = [
                            'type' => 'error', // SweetAlert icon
                            'message' => 'Password not Changed! Try Again!' // SweetAlert title
                        ];
                    }
                } else {
                    $_SESSION['notification'] = [
                        'type' => 'warning', // SweetAlert icon
                        'message' => "Passwords don't match." // SweetAlert title
                    ];
                }
            } else {
                $_SESSION['notification'] = [
                    'type' => 'error', // SweetAlert icon
                    'message' => 'Your current password is incorrect.' // SweetAlert title
                ];
            }
        } else {
            $_SESSION['notification'] = [
                'type' => 'error', // SweetAlert icon
                'message' => "Account doesn't exist." // SweetAlert title
            ];
        }
    } else {
        $_SESSION['notification'] = [
            'type' => 'warning', // SweetAlert icon
            'message' => 'Please fill out all the fields.' // SweetAlert title
        ];
    }

    // Redirect to admin.php to display SweetAlert
        ...
        <div class="card position-relative border border-white">
          ...
          <div class="card-body p-4 text-start text-white border border-dark border-2" style="background-color: #296aeb; padding-top: 80px; border-radius:2rem;">
            <div class="mx-4">
              ...
              <form action="changerpass.php" method="POST">
              ...
              <button onclick="location.href='registrar.php'" class="btn btn-info mt-2 p-2">
                Cancel
              </button>
