...
if(isset($_POST['account_add'])){
    ...
    $sql = "SELECT student_id FROM enroll WHERE id = '$id'";
    ...
    if ($statement->rowCount() > 0) {
        ...
        $query = "INSERT INTO `users`(`id`, `username`, `password`, `firstname`, `lastname`) 
              SELECT id, student_id, birthday, firstname, lastname FROM enroll WHERE id = '$id'";
        $statement = $conn->prepare($query);
        $statement->execute();

    // Fetch email address from enroll table
        $emailQuery = "SELECT emailaddress FROM enroll WHERE student_id = '$username'";
        ...
        try {
            ...
            $mail->addAddress($studentEmail); // Use student's email from enroll table

            $mail->isHTML(true);
            $mail->Subject = 'Your Grace Baptist Academy Account Details';
            $mail->Body = "Dear Student,<br><br>Your account has been successfully created. Below are your account details:<br>
                          <strong>Username:</strong> $username<br>
                          <strong>Password:</strong> [Your Birthday][YYYY-MM-DD]<br><br>
                          Please log in to your account to update your password and complete the process.<br><br>
                          Best regards,<br>Grace Baptist Academy Admissions Office";

            $mail->send();

            // Set success message
            $_SESSION['error'] = "Account Added and Email Sent!";
            $_SESSION['status'] = "success";
            $_SESSION['title'] = "Success!";
        } catch (Exception $e) {
            // Set error message for failed email
            $_SESSION['error'] = "Account Added, but email could not be sent. Mailer Error: {$mail->ErrorInfo}";
            $_SESSION['status'] = "warning";
            $_SESSION['title'] = "Email Error!";
        }
    } else {
        // Set error message if email not found
        $_SESSION['error'] = "Account Added, but no email address found.";
        $_SESSION['status'] = "warning";
