    ...
    if (in_array($imageExt, $allowedExtensions) && $imageError === 0 && $imageSize < 5000000) {
        ...
        if (move_uploaded_file($imageTmpName, $imageDestination)) {
            try { 
                ...
                // Now insert into the payments table with the session ID as admission_id
                $sql = "INSERT INTO payments (admission_id, fullName, referenceNumber, amount, receiptImage, payment_mode, payment_terms, due_date) 
                        VALUES (:admission_id, :fullName, :referenceNumber, :amount, :receiptImage, :paymentMode, :paymentTerms, :due_date)";
                $stmt = $conn->prepare($sql);

                // Bind parameters for payments
                $stmt->bindValue(':admission_id', $sessionId);
                ...
                $stmt->bindValue(':paymentTerms', $paymentTerms);
                ...
                if ($stmt->execute()) {
                    ...
                    $emailSql = "SELECT emailaddress FROM enroll WHERE id = :id";
                    $emailStmt = $conn->prepare($emailSql);
                    $emailStmt->bindValue(':id', $sessionId); // Assuming sessionId is the admission ID
                    $emailStmt->execute();
                    ...
                    if ($paymentTerms === 'installment' || $paymentTerms === 'full') {
                        ...
                        $updateStmt->bindValue(':admission_id', $sessionId);
                    ...
                    echo "<script>
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    window.location.href = 'student1.php'; // Redirect to index.php
                                }
                            });
                          </script>";
                } else {
